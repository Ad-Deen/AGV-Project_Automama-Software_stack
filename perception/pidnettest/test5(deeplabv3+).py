...
CITYSCAPES_COLORMAP = np.array([
    ...
    [  0,   0,  0],  # 19 - void/unlabeled or background
    [128,128,  0],  # 20 - extra
    [128,  0,128],  # 21 - extra
    [  0,128,128],  # 22 - extra
    [128,128,128],  # 23 - extra
    [ 64,  0,128],  # 24 - extra
    [192,192,128],  # 25 - extra
    [192,  0, 64],  # 26 - extra
    [ 64,192,  0],  # 27 - extra
    [ 64, 64,128],  # 28 - extra
    [  0, 64, 64],  # 29 - extra
...
class TensorRTInference:
    ...
    def preprocess_frame(self, frame):
        """Preprocess frame for NHWC model input"""
        target_h, target_w = self.input_shape[1], self.input_shape[2]  # NHWC format
        ...
        # Add batch dimension (already NHWC)
    ...
    def postprocess(self, output_data, original_frame_shape):
        ...
        output_reshaped = output_data.reshape(self.output_shape)  # (1, H, W, C)
        # print(output_reshaped.shape)
        logits = output_reshaped[0]  # (H, W, num_classes)
        print(np.min(logits[-1]))
        print(np.max(logits[-1]))

        pred = np.argmax(logits, axis=-1).astype(np.uint8)  # (H, W)
        # print(pred)

        # Apply full colormap (make sure CITYSCAPES_COLORMAP has 34 entries)
        color_mask = CITYSCAPES_COLORMAP[pred]  # shape: (H, W, 3)
...
def main():
    # /home/deen/ros2_ws/src/automama/automama/perception/deeplabv3+_fp32_200x400.engine
    # Initialize TensorRT inference
    engine_path = "/home/deen/ros2_ws/src/automama/automama/perception/deeplabv3+_fp32_200x400.engine"
    ...
    try:
        ...
        while True:
            ...
            # Create overlay
            blended = cv2.addWeighted(frame, 0.3, mask, 0.7, 0)
