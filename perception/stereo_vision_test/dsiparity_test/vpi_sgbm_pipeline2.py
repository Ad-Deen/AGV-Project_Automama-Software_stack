...
def main():
    ...
    # gray = cv2.cvtColor(np_left, cv2.COLOR_BGR2GRAY)
    # cv2.imshow("im",gray)
    # cv2.waitKey(0) 
    ...
    # gray = cv2.cvtColor(np_right, cv2.COLOR_BGR2GRAY)
    # cv2.imshow("im",gray)
    # cv2.waitKey(0)
    ...
    if args.backend in {'ofa-pva-vic', 'ofa'}:
        ...
        with vpi.Backend.VIC:
            with streamLeft:
                ...
                # left_cv = left.convert(vpi.Format.U8).cpu()

            with streamRight:
                right = right.convert(vpi.Format.Y16_ER_BL)
                # right_cv = right.convert(vpi.Format.U8).cpu()
    if args.verbose:
    ...
    with streamStereo, backend:
        disparityS16 = vpi.stereodisp(left, right, downscale=downscale, out_confmap=confidenceU16,
                                    ...
                                    p1=5, p2=190, p2alpha=0,
                                    includediagonals=3, numpasses=3)
    ...
    disp = cv2.resize(disparityU8,(int(disparityU8.shape[1]*down),int(disparityU8.shape[0]*down)))
    cv2.imshow("im",disp)
    cv2.waitKey(0)
    ...
    #         Image.fromarray(disparityU8).save(disparityU8)
    #         if args.verbose:
    ...
    #         disparityS16.cpu().tofile(disparityU8)
    #         if args.verbose:
    ...
    # except:
    #     raise ValueError(f'E Cannot write outputs: {disparityU8}, {disparityU8}\n'
