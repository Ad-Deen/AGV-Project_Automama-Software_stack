...
# -------- CUDA kernel (parameterized) --------
_EUCLIDEAN_GRADIENT_KERNEL = r"""
extern "C" __global__
void euclidean_gradient_kernel(
    unsigned char* __restrict__ costmap,
    const int rows,
    const int cols,
    const int start_x,
    const int start_y,
    const float scale,                 // e.g., 0.1
    const unsigned char update_below,  // only update if pixel < update_below (e.g., 250)
    const unsigned char cap_value      // clamp result to <= cap_value (e.g., 245)
){
    ...
    // Update only if current cost < update_below
    unsigned char curr = costmap[index];
    if (curr < update_below) {
        float dx = (float)x - (float)start_x;
        float dy = (float)y - (float)start_y;
        float distance = sqrtf(dx*dx + dy*dy);

        // Increment scaled by distance
        int inc = (int)lrintf(scale * distance);
        int new_cost = (int)curr + inc;

        // Clamp to cap_value
        if (new_cost > (int)cap_value) new_cost = (int)cap_value;

        costmap[index] = (unsigned char)new_cost;
    }
}
""";

# Compile once
_euclidean_kernel = cp.RawKernel(_EUCLIDEAN_GRADIENT_KERNEL, "euclidean_gradient_kernel")


def euclidean_costmap(costmap_cp: cp.ndarray,
                      start_xy: tuple[int,int] | None = None,
                      scale: float = 0.1,
                      update_below: int = 250,
                      cap_value: int = 245,
                      in_place: bool = True) -> cp.ndarray:
    """
    Add a Euclidean-distance-based gradient to a uint8 costmap.

    Args:
        costmap_cp   : (H, W) CuPy uint8 array (modified in place unless in_place=False)
        start_xy     : (x, y) start point; default = (W//2, H-1) (bottom center)
        scale        : multiplier for distance -> increment (e.g., 0.1)
        update_below : only update pixels whose current value < this (e.g., 250)
        cap_value    : clamp final cost to this max (e.g., 245)
        in_place     : if False, works on a copy and returns it

    Returns:
        Modified costmap as CuPy array.
    """
    if costmap_cp.dtype != cp.uint8:
        costmap_cp = costmap_cp.astype(cp.uint8, copy=not in_place)

    if not in_place:
        costmap_cp = costmap_cp.copy()

    rows, cols = map(int, costmap_cp.shape)
    if rows == 0 or cols == 0:
        return costmap_cp

    # Default start: mid-bottom
    if start_xy is None:
        start_x = cols // 2
        start_y = rows - 1
    else:
    ...
    # Launch config
    tpbx, tpby = 16, 16
    bpgx = (cols + tpbx - 1) // tpbx
    bpgy = (rows + tpby - 1) // tpby
...
# Updated DT kernel with separate horizontal and vertical sizes
dt_kernel_code = r'''
extern "C" __global__
               ...
               const int h_size, const int v_size)
{
    ...
    float min_dist = 1e20f;  // Initialize very large

    // Search upper half with separate h_size / v_size
    for (int dy = -v_size; dy <= 0; ++dy) {
        for (int dx = -h_size; dx <= h_size; ++dx) {
            int nx = x + dx;
            int ny = y + dy;
    ...
    // Map distance to [0,255], 0 at obstacle, max at farthest free
    float normalized = fminf(min_dist / (float)v_size, 1.0f);  // normalize by vertical size
...
def distance_transform(costmap: cp.ndarray, h_size: int = 60, v_size: int = 20):
    """
    Distance Transform with separate horizontal and vertical kernel sizes.
    ...
    dt_kernel(
        ...
        threadsperblock,
        (costmap, dtmap, H, W, h_size, v_size)
    )
...
class NavStack:
    ...
    def preprocess_costmap(self, costmap_cp):
        ...
        target_dt = distance_transform(edged)
