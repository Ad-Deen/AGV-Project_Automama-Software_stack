...
void euclidean_gradient_kernel(unsigned char* costmap,
    ...
    if (x < cols && y < rows) {
        ...
        if (costmap[index] < 100) {
            ...
            int new_cost = costmap[index] + (int)euclidean_cost * 1; //scaling factor = 1.0
...
void euclidean_gradient_target_kernel(unsigned char* costmap,
    ...
    if (x < cols && y < rows) {
        ...
        if (costmap[index] < 200) {
            ...
            int new_cost = costmap[index] + (int)(distance) *1.5; 
...
def euclidean_costmap_from_target(costmap_cp, target_cp):
    ...
    """
    rows, cols = costmap_cp.shape
    target_y, target_x = target_cp.get()
...
class NavStack:
    ...
    def preprocess_costmap(self, costmap_cp):
        ...
        rows, cols = gradient_costmap.shape
        ...
        # print(gradient_costmap.get())
        # cv2.imshow('gradient_costmap',gradient_costmap.get())

        cv2.imshow('euclidean_costmap_cp',euclidean_costmap_cp.get())
        if euclidean_costmap_cp is not None :
            targ = cp.argwhere(euclidean_costmap_cp < 50)[0]
        #  ================   checking if the target is correct ====================
        # print(targ)
        # costmap_vis = CostmapVisualizer.add_markers(euclidean_costmap_cp,[start_x,start_y],targ,15)
        if targ.size != 0:
