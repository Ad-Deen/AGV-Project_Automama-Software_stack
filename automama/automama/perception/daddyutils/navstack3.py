...
class NavStack:
    ...
    def preprocess_costmap(self, costmap_cp):
        ...
        including a target smoothing mechanism.
        ...
        # Check if there are any drivable paths (pixels with value 0)
        potential_targets = cp.argwhere(costmap_cp == 0)
        if potential_targets.size == 0:
            print("Warning: Costmap has no drivable paths (value 0). Cannot find a target.")
            return None  # Return None to prevent further processing and the IndexError

        # --- Start of new target smoothing logic ---
        # Define the threshold for the x-coordinate (column).
        # This value controls how much the target can shift horizontally.
        

        # Case 1: This is the very first time we're setting a target.
        # There is no previous target to compare against.
        if not hasattr(self, 'prev_target') or self.prev_target is None:
            # Simply pick the first potential target and set it as the current target.
            self.target = potential_targets[0]
            print(f"Initial target set: {self.target.tolist()}")
        # Case 2: We have a previous target to compare against.
        else:
            # Store a list of potential targets that are within the x_threshold.
            prev_x = self.prev_target[1]

            # Use CuPy to efficiently filter targets on the GPU.
            ...
            # If we found one or more targets that meet the threshold...
            if filtered_targets.size > 0:
                # We can choose the best one from the filtered list.
                # Here, we just choose the first one.
                ...
                # we should still choose the best available one (the first one)
                # from the unfiltered list to prevent the vehicle from getting stuck.
                self.target = potential_targets[0]
                print(f"No targets within threshold, falling back to best available: {self.target.tolist()}")

        # Update the prev_target for the next iteration. We store a regular list/tuple.
        ...
        if euclidean_costmap_cp.size == 0:
            ...
            # The new self.target is already correctly set, so we can use its column value.
