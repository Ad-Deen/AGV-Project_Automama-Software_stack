...
def compute_avg_steering_costmap(costmap_cp: cp.ndarray,
    ...
    Args:
        ...
        start_row: row of vehicle front
    ...
    # Sort by row ascending (from bottom to top in map coordinates)
    sort_idx = cp.argsort(traj_rows)
    traj_rows = traj_rows[sort_idx]
    traj_cols = traj_cols[sort_idx]
    # Take first N points above start_row
    mask = traj_rows < start_row  # trajectory above vehicle
    sel_rows = traj_rows[mask][:lookahead_points]
    sel_cols = traj_cols[mask][:lookahead_points]

    if sel_rows.size == 0:
        return 0.0

    # Compute opposite and adjacent
    opposite = sel_cols - vehicle_center_col
    adjacent = start_row - sel_rows  # vertical distance
