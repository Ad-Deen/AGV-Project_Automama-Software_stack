        ...
        self.inflation_kernel = cp.RawKernel(r'''
            extern "C" __global__ void inflate_costmap(unsigned char* costmap, int rows, int cols, int h_step, int v_step) {
                ...
                if (x < cols && y < rows) {
                    ...
                    if (costmap[index] == 0) {
                        bool found_occupied_horizontally = false;
                        
                        // --- Horizontal check for occupied pixels
                        // Check h_step to the left
                        if (x - h_step >= 0 && costmap[index - h_step] == 255) {
                            found_occupied_horizontally = true;
                        }
                        // Check h_step to the right
                        if (x + h_step < cols && costmap[index + h_step] == 255) {
                            found_occupied_horizontally = true;
                        }

                        if (found_occupied_horizontally) {
                            costmap[index] = 200;
                        } else {
                            // If no occupied pixels were found horizontally, perform the vertical check
                            bool found_free_space_vertically = false;
                            
                            // --- Vertical check (upward only)
                            for (int i = 1; i <= 7; ++i) {
                                if (y - (i + v_step) >= 0) {
                                    // Check for a pixel that is free space (i.e., == 0)
                                    if (costmap[index - (i + v_step) * cols] == 0) {
                                        found_free_space_vertically = true;
                            ...
                            if (!found_free_space_vertically) {
                                costmap[index] = 231; // Did not find free space upward, so this pixel is inflated
                            }
                        }
                    }
                }
