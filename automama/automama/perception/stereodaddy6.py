...
class StereoShowNode(Node):
    def __init__(self):
        ...
        self.step = 2             # Sparse block step (1 = dense, >1 = sparse)
        self.p1 = 0
        self.p2 = 0
    ...
    def timer_callback(self):
        ...
        # left_mask = mask_red_pixels(left_frame,90)
...
# --------------------Create the masked left frame on GPU--------------------------
        ...
        with cp.cuda.Device(0):
            ...
            depth_map_vis = np.clip(depth_map, 0.0, 5.0)
        ...
        #         left = vpi.asimage(masked_left_frame_np).convert(vpi.Format.U8).eqhist().convert(vpi.Format.Y16_ER).convert(vpi.Format.Y16_ER_BL,backend=vpi.Backend.VIC )
        #     #         # .rescale((256*3, 256*2), interp=vpi.Interp.LINEAR)
        #     with self.streamRight:
        #         # right_image = vpi.asimage(right_frame)
        #         right = vpi.asimage(buffered_right).convert(vpi.Format.U8).eqhist().convert(vpi.Format.Y16_ER).convert(vpi.Format.Y16_ER_BL,backend=vpi.Backend.VIC)
        #                     # .rescale((256*3, 256*2), interp=vpi.Interp.LINEAR)
        ...
        #                 .convert(vpi.Format.U16, scale=1.0/(32*128)*255)\
        ...
        # # Visualization (unchanged)
        # depth_map_vis = np.clip(depth_map, 0.0, max_depth)
        # depth_map_vis = ((depth_map_vis / max_depth) * 255.0).astype(np.uint8)
        # cv2.imshow("Disparity Map", disparity_img.astype(np.uint8))
        # cv2.imshow("Depth Map (Clipped)", depth_map_vis)
