...
from daddyutils.occupancy6 import ColorMappedDepthMapCreator
from daddyutils.navstack12 import NavStack, CostmapVisualizer
...
class StereoShowNode(Node):
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture("/home/deen/ros2_ws/src/automama/automama/perception/saved_vids/Vehicle_mounted_1.mp4")
        # self.cap = cv2.VideoCapture("/home/deen/ros2_ws/src/automama/automama/perception/Vehicle_mounted.mp4")
        ...
        # --------------Occupancy grid --------------------------
        self.grid_creator = ColorMappedDepthMapCreator(
                                    camera_height_cm= 99,
                                    vfov_deg=35.9012,   #
                                    hfov_deg=33.8805,   #33.8805
                                    pitch_deg=9,    #-4.62
                                    image_width=640,
                                    image_height=480,
                                    )
        self.navstack = NavStack()
        # self.costmap_vis = CostmapVisualizer()
    ...
    def timer_callback(self):
        ...
        # ret, left_frame = self.cap.read()    #video check
        ...
        left_mask = self.trt_infer.infer(left_frame)
        

        # left_mask = mask_red_pixels(left_frame,100)
        # cv2.imshow("Inference", left_mask)
# --------------------Create the masked left frame on GPU--------------------------
        # --------------------Create the masked frames on GPU--------------------------
        with cp.cuda.Device(0):
            ...
            # print(color_mask_cp[240,:])
            # --- Generate the new binary masks on the GPU ---
            # We first create the masks for all colored pixels and for only grey pixels.

            # 1. Create a binary mask for pixels that are ANY color (any non-black pixel)
            any_color_mask_cp = cp.any(color_mask_cp > 0, axis=-1)

            # 2. Create a binary mask for pixels that are EXACTLY grey (128, 128, 128)
            grey_color_cp = cp.array([128, 128, 128], dtype=cp.uint8)
            road_mask_cp = cp.all(color_mask_cp == grey_color_cp, axis=-1)
            road_mask_cp[:140, :] = 0

            # 3. Create a binary mask for all colored pixels that are NOT grey
            # We use a bitwise AND operation to find pixels that are "any color" AND "not grey".
            other_color_mask_cp = any_color_mask_cp & ~road_mask_cp

            #------------- Occupancy grid ------------------------
            # Now, call the method from the class you instantiated earlier
            depth_np , depthmap_cp = self.grid_creator.create_color_mapped_depth_map(road_mask_cp)
            occupancy_grid_vis,occupancy_grid_cp = self.grid_creator.create_fixed_birdseye_view_depth_map(depthmap_cp)
            costmap_cp = self.grid_creator.create_dynamic_costmap(occupancy_grid_cp)
            costmap_vis, steering ,throttle  = self.navstack.preprocess_costmap(costmap_cp=costmap_cp)
            
            # self.get_logger().info(f"Debug message: {steering}")
            if steering is not None:
                self.msg.data = [float(throttle), float(steering), 0.0]  # throttle, steering, brake
                self.publisher_.publish(self.msg)
            else:
                # fallback if steering is None or invalid
                self.msg.data = [0.0, 0.0, 0.0]
                self.publisher_.publish(self.msg)
            # if occupancy_grid_cp is not None:
                # print(occupancy_grid_cp.shape)
            # --- Create the 3-channel masks for element-wise multiplication ---
            
            # Create a 3-channel binary mask from the grey mask
            grey_binary_mask_3ch = cp.stack([road_mask_cp, road_mask_cp, road_mask_cp], axis=-1)

            # Create a 3-channel binary mask from the "other" color mask
            other_color_binary_mask_3ch = cp.stack([other_color_mask_cp, other_color_mask_cp, other_color_mask_cp], axis=-1)

            # --- Apply the masks to the original frame ---
            # We apply the two specific masks to get the final masked frames.

            # Apply the grey mask to the left frame
            road_mask_cp = left_frame_cp * grey_binary_mask_3ch

            # Apply the "other" color mask to the left frame
            object_mask_cp = left_frame_cp * other_color_binary_mask_3ch

            # Get the results back to the CPU for display and further processing
            road_mask_np = road_mask_cp.get()
            object_mask_np = object_mask_cp.get()
            # costmap_np = costmap_cp.get()
        # Show the final masked frame
        # cv2.imshow("object_mask_np", object_mask_np)
        cv2.imshow("road_mask_np", road_mask_np)
        # --------------- Visualizing Depth map--------------------------
        # depth_norm_cp = ((depth_np / 50.0) * 255.0).astype(cp.uint8)   # 0 m → 0, 50 m → 255
        # depth_norm_cp = depth_norm_cp.astype(cp.uint8) 
        
        # cv2.imshow("depth map", depth_norm_cp)
        # occupancy_grid_np = cv2.resize(occupancy_grid_np,(640,480))
        # cv2.imshow("occupancy_grid_np", occupancy_grid_vis)
  
        if costmap_vis is not None :
            cv2.imshow("costmap_np", costmap_vis.get())
